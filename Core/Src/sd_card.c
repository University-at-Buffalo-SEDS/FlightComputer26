...
static UINT ensure_media_open(void) {
  if (g_media_open) return FX_SUCCESS;
  if (!g_driver_entry) return FX_PTR_ERROR;

  UINT st = ensure_fx_ready();
  if (st != FX_SUCCESS) return st;

  st = fx_media_open(&g_sd_media,
                     "SD",
                     g_driver_entry,
                     g_driver_info,
                     g_fx_media_buffer,
                     sizeof(g_fx_media_buffer));
  if (st != FX_SUCCESS) return st;
...
static UINT ensure_file_open(void) {
  ...
  UINT st = fx_file_open(&g_sd_media,
                         &g_sd_file,
                         (CHAR *)g_filename,
                         FX_OPEN_FOR_WRITE);
...
static VOID sd_log_thread_entry(ULONG arg) {
  ...
  for (;;) {
    ULONG msg = 0;
    if (tx_queue_receive(&g_sd_log_queue, &msg, TX_WAIT_FOREVER) != TX_SUCCESS) {
      continue;
    }
...
UINT sd_logger_init(const CHAR *filename, SdFxDriverEntry driver_entry, VOID *driver_info) {
  ...
  if (!filename || !driver_entry) return FX_PTR_ERROR;
  ...
  tx_queue_create(&g_sd_log_queue,
                  "sd_log_queue",
                  TX_1_ULONG,
                  g_sd_log_queue_storage,
                  sizeof(g_sd_log_queue_storage));

  tx_thread_create(&g_sd_log_thread,
                   "sd_log_thread",
                   sd_log_thread_entry,
                   0,
                   g_sd_log_thread_stack,
                   sizeof(g_sd_log_thread_stack),
                   20, 20,
                   TX_NO_TIME_SLICE,
                   TX_AUTO_START);
...
UINT sd_logger_enqueue_line(const CHAR *data, size_t len) {
  if (!g_sd_logger_up) return FX_NOT_AVAILABLE;
  if (!data) return FX_PTR_ERROR;

  sd_line_t *rec = sd_pool_alloc();
  if (!rec) return FX_NO_MORE_SPACE;

  size_t n = len;
  if (n > (SD_LOG_LINE_MAX - 2)) n = (SD_LOG_LINE_MAX - 2);
